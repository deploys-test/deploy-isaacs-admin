...
<head>
  ...
  //GUARDAR
      if (isset($_REQUEST['Guardar'])){
        ...
        $Guardar="insert into ciudades(Ciud_Id, Ciud_Nombre)values($Codigo, '$Nombre')";
  ...
  //CONSULTAR
      if (isset($_REQUEST['Consultar'])) {
        ...
        if (!empty($_REQUEST['Codigo'])) {
          $Consultar="Select * from ciudades where Ciud_Id=$Codigo";
          $respuesta=mysqli_query($link,$Consultar);
          $frow=mysqli_fetch_array($respuesta, MYSQLI_ASSOC);
          if (empty($frow)) {
          echo"<script languaje='javascript'>alert('Ciudad Inexistente')</script>";
          }
          else{
            $Codigo=$frow['Ciud_Id'];
            $Nombre=$frow['Ciud_Nombre'];
          }
        }
        else {
          echo "<script languaje='javascript'>alert('Porfavor ingrese el codigo')</script>";
        }
      }
  ...
  //ACTUALIZAR
      if(isset($_REQUEST['Actualizar'])){
        ...
        if(!empty($Codigo)){
          if(!empty($Nombre)){
            $Actualizar="Update ciudades set Ciud_Nombre='$Nombre' where Ciud_Id=$Codigo";
  ...
  //ELIMINAR
     if(isset($_REQUEST['Eliminar'])){
      $Codigo=$_REQUEST['Codigo'];
      if(!empty($Codigo)){
        $Eliminar="Delete from ciudades Where Ciud_Id=$Codigo";
        mysqli_query($link,$Eliminar);
        echo "<script languaje='javascript'>alert('Datos eliminados correctamente')</script>";
        $Codigo="";
      }
      else {
        echo "<script languaje='javascript'>alert('Porfavor ingrese el codigo de la ciudad a eliminar')</script>";
      }
     }
   ?>
</head>
...
<!--
          <div class="xCiudades" >
            <form action="" class="Class" method="post">
             ...
             <table border="0" bordercolor="" width ="10%" cellspacing="15" cellpadding="4">
                <h1 class="Nombres"> Ciudades </h1>
								<hr class="Linea" align="center" noshade="noshade" size="2" width="90%" />
                <tr> <td><font size="4" class="Titulos">Codigo:<td><input type="text" maxlength="2" class="Nombres" name="Codigo" value="<?php echo $Codigo; ?>"> <br></tr>
                <tr> <td><font size="4" class="Titulos">Nombre:<td><input type="text" maxlength="25" class="Nombres" name="Nombre" value="<?php echo $Nombre; ?>"> <br></tr>
             </table>
                <hr class="Linea" align="center" noshade="noshade" size="2" width="90%" />
 ...
 <!-- Aqui va el listar -->
			...
			<iframe class="container" src="listar\Ciudades\index.php" width="1000px" height="500"></iframe>
		  <!--	</section> -->
				<br>
